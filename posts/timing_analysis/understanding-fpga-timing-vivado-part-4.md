...
$$T_{required} = T_{period} + T_{clk,\,capture} - T_{su,\,int} - \text{clock uncertainty} \tag{4}$$
...
where $T_{arrival,\,min} = \text{input\_delay}_{min} + T_{route,\,int,\,min}$ and $T_{required,\,hold} = T_{clk,\,capture,\,min} + T_{hold,\,int}$.
